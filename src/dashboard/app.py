...
.kpi-card:hover { border-color: rgba(0,255,136,0.25); }
.kpi-card::after {
    content: '';
    position: absolute;
...
.kpi-card.blue::after   { background: linear-gradient(90deg, #4d9fff, transparent); }
.kpi-label {
    font-size: 0.72rem;
    font-weight: 600;
    letter-spacing: 2px;
...
.event-card:hover { background: #112540; }
...
def generate_risk_trend(base: float, days: int = 30, seed: int = 42) -> pd.DataFrame:
...
def get_company_seed(company: str) -> int:
    return sum(ord(c) for c in company) % 1000

def get_base_risk(company: str) -> float:
...
# ── Try real DB, fall back to demo ────────────────────────────────────────────
USE_DEMO = True
try:
    from src.utils.db import initialize_db, get_daily_risk, get_latest_events, get_company_snapshot
    initialize_db()
    USE_DEMO = False
except Exception:
    pass
...
with st.sidebar:
    ...
    <div style="text-align:center; padding: 1rem 0 0.5rem;">
        ...
        <div style="font-family:'Syne',sans-serif; font-size:1.1rem; font-weight:800;
                    color:#fff; letter-spacing:-0.3px;">ESG Risk Monitor</div>
        <div style="font-size:0.7rem; color:rgba(255,255,255,0.3); letter-spacing:2px;
                    text-transform:uppercase; margin-top:0.2rem;">Powered by FinBERT-ESG</div>
    </div>
    """, unsafe_allow_html=True)
    ...
    <div style="text-align:center; padding:1.5rem 0;">
        <div style="font-size:0.65rem; letter-spacing:2px; text-transform:uppercase;
                    color:rgba(255,255,255,0.3); margin-bottom:0.5rem;">Current Risk Score</div>
        <div style="font-family:'Syne',sans-serif; font-size:3.5rem; font-weight:800;
                    color:{color}; line-height:1;">{latest_score:.2f}</div>
    ...
    if st.button("🔄  Refresh Live Data", use_container_width=True):
        with st.spinner("Running ESG pipeline…"):
            try:
                ...
                run_pipeline(companies=[selected])
                st.success("Updated!")
            except Exception as e:
                st.warning(f"Demo mode — add NEWS_API_KEY to enable live data.")

    st.markdown("""
    <div style="margin-top:2rem; font-size:0.68rem; color:rgba(255,255,255,0.2);
                text-align:center; line-height:1.8;">
        FinBERT-ESG · NewsAPI · Streamlit<br>
        Scikit-learn · SQLite · Plotly
    </div>
    """, unsafe_allow_html=True)
...
if USE_DEMO:
    ...
    <div class="demo-banner">
        ⚡ <strong>Demo Mode</strong> — Showing simulated ESG data.
        Add <code>NEWS_API_KEY</code> in Streamlit Cloud secrets for live news analysis.
    </div>
    """, unsafe_allow_html=True)
...
<div class="hero-banner">
    ...
    <div class="hero-subtitle">
        ...
        NLP-powered by FinBERT-ESG &nbsp;|&nbsp; Updated {date.today().strftime('%b %d, %Y')}
    </div>
</div>
""", unsafe_allow_html=True)
...
with col_chart:
    ...
    fig.add_trace(go.Scatter(
        ...
        name="Composite Risk", line=dict(color="#00ff88", width=2.5),
        mode="lines", fill="tozeroy",
        fillcolor="rgba(0,255,136,0.04)",
    ))
    for col_name, color, label in [
        ("env_score", "#00ff88", "Environmental"),
        ("social_score", "#4d9fff", "Social"),
        ("gov_score", "#b47fff", "Governance"),
    ]:
        fig.add_trace(go.Scatter(
            x=df_risk["date"], y=df_risk[col_name],
            name=label, line=dict(color=color, dash="dot", width=1.2),
            opacity=0.6,
        ))
    fig.add_trace(go.Scatter(
        x=df_fc["date"], y=df_fc["risk_score"],
        name="Forecast", line=dict(color="#ff4d6d", dash="dash", width=2),
    ))
    fig.add_vrect(x0=df_fc["date"].iloc[0], x1=df_fc["date"].iloc[-1],
    ...
    fig.update_layout(
        ...
        yaxis=dict(showgrid=True, gridcolor="rgba(255,255,255,0.05)",
                   range=[0, 1], color="rgba(255,255,255,0.2)"),
...
with col_pillar:
    ...
    pillars_df = pd.DataFrame({
        ...
        "Score": [env, soc, gov],
        "Color": ["#00ff88", "#4d9fff", "#b47fff"],
    })
    fig2 = go.Figure()
    for _, row in pillars_df.iterrows():
        fig2.add_trace(go.Bar(
            x=[row["Score"]], y=[row["Pillar"]],
            orientation="h", name=row["Pillar"],
...
for idx in sample_events:
    label, sentiment, headline, risk = DEMO_EVENTS[idx]
    css_class = {"Environmental": "env", "Social": "soc", "Governance": "gov"}.get(label, "env")
    tag_class  = {"Environmental": "tag-env", "Social": "tag-soc", "Governance": "tag-gov"}.get(label, "")
    sent_class = {"negative": "tag-neg", "neutral": "tag-neu", "positive": "tag-pos"}.get(sentiment, "")
    sent_icon  = {"negative": "⚠️", "neutral": "➖", "positive": "✅"}.get(sentiment, "")
    days_ago   = int(rng2.integers(0, 7))
    event_date = (date.today() - timedelta(days=days_ago)).strftime("%b %d")
    events_html += f"""
    <div class="event-card {css_class}">
        <div class="event-title">{headline}</div>
        <div class="event-meta">
            <span class="event-tag {tag_class}">{label}</span>
            <span class="event-tag {sent_class}">{sent_icon} {sentiment}</span>
            <span>{event_date}</span>
            <span>Risk: <strong style="color:{'#ff4d6d' if risk>0.6 else '#ffd700' if risk>0.3 else '#00ff88'}">{risk:.2f}</strong></span>
        </div>
    </div>"""
...
leaderboard = sorted(
    [(c, get_base_risk(c) * 0.9 + generate_risk_trend(get_base_risk(c), seed=get_company_seed(c))["risk_score"].iloc[-1] * 0.1)
     for c in COMPANIES],
    ...
    with col:
        ...
        <div class="lb-row">
            ...
            <div class="lb-bar-wrap">
                <div class="lb-bar" style="width:{bar_pct}%; background:{color};"></div>
            </div>
            <div class="lb-score" style="color:{color}">{score:.2f}</div>
        </div>
        """, unsafe_allow_html=True)
